...
- 修复交易分析中心「指数数据滞后一个月导致选股/择时信号不完整」的问题：`indices`（沪深300）此前从不参与增量更新，一直停在数据湖初始日期，导致月/周/日三层择时与个股快照日期脱节。
- 新增指数增量更新能力：`incremental_update` / 每日脚本 `update_daily.py` 现在同时补齐指数日线；数据管理页新增「补齐指数日线」按钮，状态栏展示指数数据日期与滞后提示。
- 交易分析中心生成建议前自动补齐指数数据（`ensure_index_fresh`），并在盘面环境里显式标注「指数数据滞后」而非把陈旧收盘价伪装成最新价。
- 大模型分析过程增强：
  - **自主补全数据**：新增 `update_index_data` / `update_stock_data` 工具，分析中发现指数/个股数据滞后或缺失时可主动补全。
  - **自主调用工具**：选股循环从 5 轮扩到 8 轮，每轮动态刷新工具 schema（新造工具可被后续轮次立即使用），选股 `max_tokens` 调大到 8000。
  - **自主创造工具**：新增 `create_sql_tool` 工具，分析过程中可用自然语言或显式 SQL 创造只读查询工具，校验后即时注册并持久化。
- 抽取统一的 SQL 工具生成助手（`tools/generator.py`），供手动「生成工具」与分析中「自主造工具」复用。
...
- 修复数据湖近几日（08-17 / 08-18）部分「收盘价」被盘中手动增量更新污染为盘中价的历史数据质量问题：增量更新与手动脚本改为仅在收盘后（北京时间 15:00 后）落库，盘中不再写入。
...
- [发布说明 v0.2.2](docs/releases/v0.2.2.md)
